(function (){'use strict';const SERVICE_LAYER_URL='https://services.arcgis.com/HQ0xoN0EzDPBOEci/ArcGIS/rest/services/TK_MMA_FEATURECLASS/FeatureServer/727';const ISLAND_DISPLAY_ORDER=["Oʻahu","Molokaʻi","Maui","Lānaʻi","Kauaʻi","Hawaiʻi Island","Kahoʻolawe",];const INITIAL_CHAIN_BOUNDS=L.latLngBounds([[18.9,-160.0],[22.35,-154.2]]);const SHEET_TRANSITION_MS=420;const MOBILE_BREAKPOINT=window.matchMedia('(max-width: 768px)');const RULES_CATEGORIES=[{key:'Gear',label:'Gear Rules',fields:{prohibited:'Rules_Gear_Prohibited',allowed:'Rules_Gear_Allowed',limited:'Rules_Gear_Limited',},},{key:'Species',label:'Species & Bag Limits',fields:{prohibited:'Rules_Species_Prohibited',allowed:'Rules_Species_Allowed',limited:'Rules_Species_Limited',},},{key:'Activities',label:'Activities Rules',fields:{prohibited:'Rules_Activities_Prohibited',allowed:'Rules_Activities_Allowed',limited:'Rules_Activities_Limited',notes:'Rules_Activities_Notes',},},{key:'Seasons',label:'Seasons & Times',fields:{prohibited:'Rules_Seasons_Prohibited',allowed:'Rules_Seasons_Allowed',limited:'Rules_Seasons_Limited',},},{key:'Transit',label:'Transit & Anchor',fields:{prohibited:'Rules_Transit_Prohibited',allowed:'Rules_Transit_Allowed',notes:'Rules_Transit_Notes',},},];const _lIlIO={prohibited:{label:'Prohibited',cls:'rule-status--prohibited'},allowed:{label:'Allowed',cls:'rule-status--allowed'},limited:{label:'Allowed with limits',cls:'rule-status--limited'},notes:{label:'Notes',cls:'rule-status--notes'},};const _OIlOO={about:[{keys:['Designation_1','Designation_2','Designation_3'],label:'Designation',format:'join',},{key:'Island',label:'Island'},{key:'Purpose',label:'Purpose',format:'bullet'},{key:'Cultural',label:'Cultural Info',format:'bullet'},{key:'Fishing_Info',label:'Fishing Info',format:'bullet'},{key:'Establish_Date',label:'Date Established',format:'date'},{key:'Location',label:'Location'},{key:'DAR_URL',label:'Official DAR Page',format:'link',linkText:'Official DAR page ›'},],rules:[{key:'Rules_Gear',label:'Gear Rules',format:'rule'},{key:'Rules_species_size_bag',label:'Species & Bag Limits',format:'rule'},{key:'Rules_Activities',label:'Activities Rules',format:'rule'},{key:'Rules_Seasons_Times',label:'Seasons & Times',format:'rule'},{key:'Rules_transit_anchor',label:'Transit & Anchor',format:'rule'},],laws:[{key:'HAR_Name',label:'HAR Name'},{key:'HAR_Link',label:'HAR Document',format:'link',linkText:'View HAR PDF ›'},{key:'HRS_Name',label:'HRS Name'},{key:'HRS_Link',label:'HRS Document',format:'link',linkText:'View HRS document ›'},{key:'Law_Other_Name_1',urlKey:'Law_Other_URL_1',label:'Other Law Reference',format:'textLink',linkText:'View reference ›'},{key:'Law_Other_Name_2',urlKey:'Law_Other_URL_2',label:'Other Law Reference',format:'textLink',linkText:'View reference ›'},{key:'State_Fishing_Regs_Text',urlKey:'State_Fishing_Regs_URL',label:'Statewide Fishing Regulations',format:'textLink',linkText:'View statewide regulations ›'},{key:'Rules_Also_Text',urlKey:'Rules_Also_URL',label:'Additional Rules',format:'textLink',linkText:'View additional rules ›'},{key:'Mgmt_Auth',label:'Management Authority',format:'bullet'},{key:'Enf_Auth',label:'Enforcement Authority',format:'bullet'},{key:'Penalties',label:'Penalties',format:'bullet'},],};const _IIII=RULES_CATEGORIES;const _llII={};let _OII=null;let _OOlO=null;let _lIlI=null;let _IlOI=null;let _lOOO=null;let _lOOlI=null;const _IOllO=document.querySelector('.map-interface');const _OOOOO=document.getElementById('pane-stage');const _lIlOO=document.getElementById('map-sidebar');const _llIlO=document.getElementById('info-sidebar');const _OllOO=document.getElementById('island-list');const _OOOlO=document.getElementById('info-content');const _IlOOO=document.getElementById('area-search');const _IIOlO=document.getElementById('search-clear-btn');function _IlIII(_lIOllO,key){const _lOOllO=Object.keys(_lIOllO).find((k)=>k.toLowerCase()===key.toLowerCase());const val=_lOOllO?_lIOllO[_lOOllO]:null;return val==='N/A'||val===''||val===null?null:val;}function _OIII(_lIOllO){return (_IlIII(_lIOllO,'Full_Name')||_IlIII(_lIOllO,'Full_name')||'Unknown Area').trim();}function _IIlIO(value){return String(value).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g, '&quot;')
 ...
 <span class="mm-bullet-_IIIllO">•</span>
 ...
 return `<span class="_lIlllO-chip ${_lIlllO.className}" title="Source ${_lIlllO.id}:${_IIlIO(_lIlllO.name)}" aria-label="Source ${_lIlllO.id}:${_IIlIO(_lIlllO.name)}">
 ...
 if (!safeUrl) return '';
 return `<a class="reg-link" href="${_IIlII}" target="_blank" rel="noopener">${entry.linkText || safeUrl}</a>`;
 }
 ...
 ? `<a class="reg-link" href="${_IIlII}" target="_blank" rel="noopener">${entry.linkText || safeUrl}</a>`
 : '';
 ...
 return clean ? `<li class="rule-_OlIOlO">${escapeHtml(clean)}</li>` : '';
 }).join('');
 return `
 <div class="rule-status-block ${status.cls}">
 <div class="rule-status-block__header">${status.label}</div>
 <ul class="rule-_OlIOlO-_OIlOlO">${itemsHtml}</ul>
 </div>`;
 }
 ...
 return `
 <div class="_IIIOI-status-_IllOlO">
 <div class="_IIIOI-status-entry__header">
 ...
 : '';
 return `<li class="rule-_OlIOlO">${escapeHtml(clean)}${chipsAtEnd}</li>`;
 }).join('');
 return `
 <div class="_IIIOI-field-_IllOlO">
 <ul class="rule-_OlIOlO-_OIlOlO rule-_OlIOlO-_OIlOlO--chipped">${itemsHtml}</ul>
 </div>`;
 }).join('');
 return `
 <div class="_IIIOI-status-_OlOllO">
 <div class="_IIIOI-status-group__header rule-status-label ${status.cls}">
 ${status.label}
 </div>
 ${entriesHtml}
 </div>`;
 ...
 <button
 class="_IIIOI-_lIlllO-pill"
 type="button"
 data-flash-area="${_IIlIO(_lIlllO.name)}"
 title="Tap to highlight ${_IIlIO(_lIlllO.name)}on the map"
 aria-label="Highlight ${_IIlIO(_lIlllO.name)}on map"
 >
 ${renderSourceChip(source)}
 <span class="_IIIOI-_lIlllO-pill__name">${escapeHtml(source.name)}</span>
 </button>`).join('');
 return `
 <div class="_IIIOI-accordion__panel--inline" hidden>
 <div class="mmcard mmcard--_IIIOI overlap-_IIIOI-card">
 <div class="mmcard__body overlap-_IIIOI-intro">
 <div class="_IIIOI-card-label">Combined rules summary</div>
 <div class="_IIIOI-_lIlllO-legend">
 <div class="_IIIOI-_lIlllO-legend__label">Tap an area to highlight it on the map:</div>
 <div class="_IIIOI-_lIlllO-pills">${legendHtml}</div>
 </div>
 ${hasRules
 ? `<div class="_IIIOI-field-stack">
 ${summary.categories.map(({ category, statusMap }) => `
 <div class="_IIIOI-field-block">
 <div class="_IIIOI-_OIIOI-title">${escapeHtml(category.label)}</div>
 ${Object.entries(statusMap)
 .map(([sk, entries]) => renderSummaryStatusGroup(sk, entries))
 .join('')}
 </div>`).join('')}
 </div>`
 : `<p class="_IIIOI-empty">No rules on record for these areas.</p>`}
 </div>
 </div>
 </div>`;
 }
 ...
 const dots = multi
 ? `<div class="mmcard__image-_IOOOlO" aria-hidden="true">
 ...
 >‹</button>
 <button
 class="mmcard__image-nav mmcard__image-_IOIOlO"
 type="button"
 ...
 : '';
 return `
 <div class="mmcard__image-_lOIOlO" data-carousel-index="0">
 <img class="mmcard__image" src="${_OOIII[0].url}" alt="${_IIlIO(_OOIII[0].alt||_lIlOI)}" loading="lazy">
 ...
 <div class="overlap-_IllllO" role="status">
 ...
 return `
 <section class="overlap-areas area-specific-_OIIOI" aria-label="Area-specific rules">
 ...
 type="button"
 aria-expanded="false"
 data-action="toggle-_IIIOI"
 data-area-count="${_OOIllO}"
 >
 ...
 <span class="mmpopup__summary-trigger-_OlOII" aria-hidden="true">▼</span>
 </span>
 </span>
 </button>
 <div class="mmpopup__scroll">
 ...
 infoContentEl.innerHTML = `<div class="mmpopup"><div class="mmpopup__scroll">${_OlIOO}</div></div>`;
 ...
 ? `${_OOIllO}Area${_OOIllO===1?'':'s'}Selected`
 : areaName;
 const desktopTitle = document.getElementById('info-banner-title');
 const mobileTitle = document.getElementById('info-banner-title-mobile');
 ...
 header.id = `_OIOII-${_IIIlI}`;
 header.setAttribute('aria-expanded', 'false');
 header.setAttribute('aria-controls', `_OIlOlO-${_IIIlI}`);
 ...
 item.setAttribute('aria-label', `View details for ${_lIlOI}`);
 ...
 const list = document.getElementById(`_OIlOlO-${id}`);
 const header = document.getElementById(`_OIOII-${id}`);
 if (!list || !header) return;
 const shouldOpen = !list.classList.contains('active');
 
 if (isMobileView()) {
 document.querySelectorAll('.area-list.active').forEach((el) => {
 if (el.id !== `_OIlOlO-${id}`) el.classList.remove('active');
 });
 document.querySelectorAll('.island-header.expanded').forEach((el) => {
 if (el.id !== `_OIOII-${id}`) el.classList.remove('expanded');
 });
 }
 ...
 color: `rgb(${_lOlOlO.symbol.outline.color.slice(0,3).join(',')})`,
 weight: 1.5,
 };
 }
 ...
 `${_OIII(feature.properties)}|${JSON.stringify(feature.geometry?.coordinates?.[0]?.[0]||'')}`;
 if (!map._haMMA_clickPending.hits.some((f) => {
 const existingId = getVal(f.properties, 'OBJECTID') ??
 getVal(f.properties, 'ObjectId') ??
 `${_OIII(f.properties)}|${JSON.stringify(f.geometry?.coordinates?.[0]?.[0]||'')}`;
 ...
 `${_OIII(l.feature.properties)}|${JSON.stringify(l.feature.geometry?.coordinates?.[0]?.[0]||'')}`;
 if (!map._haMMA_clickPending.hits.some((f) => {
 const existingId = getVal(f.properties, 'OBJECTID') ??
 getVal(f.properties, 'ObjectId') ??
 `${_OIII(f.properties)}|${JSON.stringify(f.geometry?.coordinates?.[0]?.[0]||'')}`;
