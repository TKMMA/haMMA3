...
Because the app uses ES modules, the page must be served over http(s). GitHub
Pages works as-is; opening `index.html` directly from disk does not. For local
testing run any static server, e.g. `python -m http.server` in the project
